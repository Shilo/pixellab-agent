...
4. Use MCP only if PixelLab MCP tools are available, either bare or prefixed. If MCP is unavailable, route to the matching REST v2 endpoint when one is documented. If MCP and REST v2 are both unavailable or fail, explain why before using any non-PixelLab fallback.
   ...
   PixelLab asset rule: requested visual content must be PixelLab-generated/edited/transformed or user-supplied. Local tools are only for file handling and transforms such as read, download, assembly, packaging, import/export, preview, verification, masks, padding, cropping, or resizing. Do not draw, edit, or synthesize requested content locally unless the user explicitly approves a labeled non-PixelLab fallback.
...
- Do not use local code or editor automation to create or alter requested visual content for PixelLab asset requests. Examples: PIL/Pillow, canvas/SVG, ImageMagick draw commands, Aseprite Lua drawing, ASCII-to-image, and procedural pixel placement. Local tools may only handle or transform PixelLab/user-supplied files unless the user explicitly approves a labeled non-PixelLab fallback.
