...
The Aseprite extension includes local skeleton reference folders for editor workflows. These names are useful research evidence for pose/keypoint concepts, but they are not documented managed-character `template_animation_id` values. Keep them separate from the public preset animation ID list unless PixelLab publishes them in REST v2 or MCP docs.
