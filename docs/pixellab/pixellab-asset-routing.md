...
| Finished skill, ability, spell, action-bar, or hotbar icon sheet | REST v2 `generate-image-v2` first. | See [PixelLab Skill Icon Generation Spike](pixellab-skill-icon-generation-spike.md): current live-generation winner is REST `generate-image-v2`, not object generation, tiles-pro, Pixen, or UI routes. Do not route to UI asset tools unless the user asks for slots, buttons, bars, panels, or containers. |
