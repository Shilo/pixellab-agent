...
| Canny/sketch, pose-guided generation, depth/image-to-image, style reference, consistent-style image creation | REST v2 closest documented image/edit route after checking current docs; Aseprite for its exact in-editor workflow. | Aseprite root WebSocket routes such as `generate-general-canny`, `generate-general-pose`, and `generate-pixelart-flux` are not public REST v2/MCP contracts. Do not call them directly from Pip. |
| Reduce colors / quantize palette | Aseprite or Pixelorama extension when the user is actively in that editor; otherwise local image tooling after explaining it is a non-PixelLab fallback. | Aseprite uses root WebSocket `quantize-image`, but no public REST v2/MCP reduce-colors endpoint was documented. Do not invent a `/v2/quantize-image` route. |
| Unzoom pixel art / remove upscaling | Aseprite or Pixelorama extension when the user is actively in that editor; otherwise local image tooling after explaining it is a non-PixelLab fallback. | Aseprite uses root WebSocket `unzoom-pixelart`, but no public REST v2/MCP Unzoom endpoint was documented. Do not invent a `/v2/unzoom-pixelart` route. |
...
| Multi image combine references or edit multiple source images | REST v2 for documented multi-image edit; website/editor for visual experimental flows. | Use `edit-images-v2` when the task is an image edit with multiple source/reference images. Do not call Aseprite root WebSocket `generate-multi-edit` directly. |
...
| Raw animation, skeleton, interpolation, outfit transfer, rotate | REST v2 unless animating a managed MCP character/object. | `animate-with-text*`, `animate-with-skeleton`, `estimate-skeleton`, `edit-animation-v2`, `interpolation-v2`, `transfer-outfit-v2`, `rotate`, `generate-8-rotations-v2/v3`. For 4 rotations, use current documented rotate/8-rotation routes or editor workflow; no public REST v2 `generate-4-rotations` route was documented. |
| Pixel correction / clean-up | Aseprite extension for its exact PixelLab workflow, or closest documented REST edit/convert route after checking docs. | Aseprite uses root WebSocket `correct-pixelart`; no public REST v2/MCP `correct-pixelart` endpoint was documented. |
| Map image, generated level image, visual map concept | REST v2 image/background route; website or Aseprite for map extension/texture workflows. | No full public REST/MCP map CRUD, map extension, or map texture surface was documented. Aseprite map/texture tools use root WebSocket routes and should not be treated as public REST. |
