...
| Portrait to character or character to portrait conversion | MCP `create_portrait_character` plus `get_portrait_character` when visible. | REST v2 `portrait-character-pro`; classify the supplied image role by `direction`: portrait input for `portrait_to_character`, full-body character input for `character_to_portrait`. This is a Pro conversion workflow, not the default text-to-managed-character route. |
| Pixel font, bitmap font, font atlas, or generated game font | MCP `create_font` plus `get_font` when visible. | REST v2 `generate-font-pro`; use concise style `description`, `weight`, `glyph_px`, `image_size`, optional `font_name`, and `seed`. This is Pro font generation, not generic image/icon generation. |
