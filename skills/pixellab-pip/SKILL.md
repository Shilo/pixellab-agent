...
| Skill icon, ability icon, spell icon, action-bar icon, or hotbar icon | REST v2 first. Do not route to object generation, tiles-pro, Pixen, or UI routes by default. | Use `generate-image-v2`, especially for complete/backgrounded icon sheets. Single backgrounded skill icons are a reasonable default but less directly tested. Prompt for pictorial symbols only, fully opaque painted backgrounds when backgrounded icons are requested, no text-like marks, no UI slots, no borders/frames/rounded corners unless the user explicitly wants slot/container art. Use UI routes only when the user asks for slots, buttons, bars, panels, or containers. |
