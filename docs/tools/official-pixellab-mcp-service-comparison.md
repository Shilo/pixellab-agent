...
Use the official MCP service when the assistant already has PixelLab MCP tools and the user wants managed PixelLab assets. Use Pip when the assistant needs setup guidance, route selection, credential safety, unsupported-flow warnings, prompt preparation, or a fallback decision between MCP and REST/API.
...
| Execution capability | Actually creates, polls, lists, downloads, and deletes PixelLab assets. | Does not provide compute by itself; it routes to MCP, REST/API, website/editor, or another documented surface. |
| Setup model | User configures an MCP client with the PixelLab MCP URL and bearer auth header. | Recommends MCP first, then guides MCP/API/manual setup with token-free previews and explicit approval before writes. |
...
| Main asset coverage | Characters, character states, animations, top-down tilesets, sidescroller tilesets, isometric tiles, tile variants, objects, map objects, balance, projects, chat, sandbox, and agent helper tools documented by MCP. | Covers MCP assets plus REST v2, website/editor-only flows, Aseprite, Pixelorama, SDK/API questions, and legacy v1 compatibility. |
...
| REST/API fallback | MCP docs point users to REST v2 when writing code or needing API surfaces not exposed through MCP. | Explicitly chooses REST v2 for scripts, batch jobs, exact endpoint control, generic images, UI, edits, resizing, background removal, and other REST-only tasks. |
...
6. Refuse or reroute website/session-token and undocumented endpoint automation.
...
Prefer REST/API guidance instead of MCP when:
...
- Do not claim MCP covers every PixelLab product surface; route REST/API and website/editor-only requests separately.
