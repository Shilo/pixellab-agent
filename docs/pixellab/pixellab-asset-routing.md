...
| Reduce colors / quantize palette | Aseprite or Pixelorama extension when the user is actively in that editor; otherwise local image tooling after labeling it as a non-PixelLab fallback. | Aseprite's observed `quantize-image` operation is an undocumented internal endpoint. No public REST v2/MCP reduce-colors endpoint was documented; do not invent `/v2/quantize-image`. |
