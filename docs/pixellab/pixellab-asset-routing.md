...
| Skill, ability, spell, action-bar, or hotbar icon | REST v2 `generate-image-v2` first. | Best-supported for complete/backgrounded sheets; reasonable default for single backgrounded skill icons, but less directly tested. See [PixelLab Skill Icon Generation Spike](pixellab-skill-icon-generation-spike.md): current live-generation winner is REST `generate-image-v2`, not object generation, tiles-pro, Pixen, or UI routes. Do not route to UI asset tools unless the user asks for slots, buttons, bars, panels, or containers. |
| General image, standalone sprite, icon, concept | REST v2 image generation. | MCP may not expose every raw image endpoint; use REST v2 for generic image primitives. For transparent item/object icons, prefer object/image routes; for backgrounded action-bar item icons, follow the skill-icon sheet guidance. |
