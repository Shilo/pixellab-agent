...
| Image edit, inpaint, mask, convert, resize, remove background | REST v2. | `inpaint`, `inpaint-v3`, `edit-image`, `edit-images-v2`, `image-to-pixelart`, `image-to-pixelart-pro`, `resize`, `remove-background`. For exact image-to-pixel-art dimensions, prefer normal `image-to-pixelart` with `output_size`; Pro currently has no output-size field. If Pro quality is required, run Pro first, then `resize` only when the target is within the resize endpoint limit; otherwise use local nearest-neighbor/canvas postprocessing after download. |
