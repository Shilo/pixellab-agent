...
2. Classify the target asset or surface:
   `general_image | skill_icon_sheet | background | character | object | effect_vfx | ui | whole_map | map_image | map_object | top_down_tileset | sidescroller_tileset | isometric_tile | tile_variants | animation | existing_image`.
...
| Finished skill, ability, spell, action-bar, or hotbar icon sheet | REST v2 first. Do not route to object generation, tiles-pro, Pixen, or UI routes by default. | Use `generate-image-v2` for complete sheets. Prompt for pictorial symbols only, invisible/borderless grid, fully opaque painted backgrounds, no text-like marks, no UI slots, no borders/frames/rounded corners. Use UI routes only when the user asks for slots, buttons, bars, panels, or containers. |
