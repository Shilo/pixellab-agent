...
Treat seamless walk loops generated from a single idle or neutral stance frame as high risk. First-frame-only attempts can produce motion, but did not reliably close the loop. Identical first/last-frame idle anchors can enforce loop pressure, but observed attempts became constrained or unpredictable across short and long prompts, negative prompting, no negative prompting, and 4-, 8-, and 16-frame requests; interpolation also tended to show palette/color shifts in the last generated frame near the endpoint. Mid-walk start/end anchors should be more reliable than idle anchors, but are not a proven complete fix. Skeleton/template routes can improve loopability and pose consistency, but observed outputs were stiff, robotic, uncanny, and prone to weird shading or hard limb shadows. Other common failures included idle collapse, mouth/talking motion, exaggerated arm movement, and weak foot contacts. Breathing/wind/smoke/spark-like artifacts near the head appear connected to inferred breathing or inferred blowing behavior when the model interprets the request as idle-like motion.
