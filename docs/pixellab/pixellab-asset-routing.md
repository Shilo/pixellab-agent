...
| Multi-image edit or combine references | REST v2 `edit-images-v2` when the task is an edit with multiple source/reference images. | Use website/editor only when the user wants the visual experimental product flow. Do not call Aseprite root WebSocket `generate-multi-edit` as a public API. |
| Canny/sketch, pose-guided image, depth/image-to-image, consistent-style generation | REST v2 closest documented image/edit endpoint after checking current docs, or Aseprite for exact in-editor behavior. | Aseprite routes such as `generate-general-canny`, `generate-general-pose`, and `generate-pixelart-flux` are extension WebSocket routes, not public REST v2 paths. |
...
| Reduce colors / quantize palette | Aseprite or Pixelorama extension when the user is actively in that editor; otherwise local image tooling after labeling it as a non-PixelLab fallback. | Aseprite uses root WebSocket `quantize-image`; no public REST v2/MCP reduce-colors endpoint was documented. |
| Unzoom pixel art | Aseprite or Pixelorama extension when the user is actively in that editor; otherwise local image tooling after labeling it as a non-PixelLab fallback. | Aseprite uses root WebSocket `unzoom-pixelart`; no public REST v2 or MCP Unzoom endpoint was documented. |
...
| Pixel correction / clean-up | Aseprite extension for exact PixelLab behavior, or closest documented REST edit/convert route after checking docs. | Aseprite uses root WebSocket `correct-pixelart`; no public REST v2/MCP endpoint with that name was documented. |
