...
6. Keep generated scripts and outputs inside the user's stated or approved output directory; when they did not choose one, use the active project/workspace `pixellab-pip/` output folder. Use another path only after explicit approval.
...
2. Download or write the verified output file under the active project/workspace `pixellab-pip/` folder unless the user explicitly states or approves another output path.
...
1. Generate or collect frames locally under the active project/workspace `pixellab-pip/` folder unless the user explicitly states or approves another output path.
...
1. Put each generated variant in a stable local path under the active project/workspace `pixellab-pip/` folder unless the user explicitly states or approves another output path.
