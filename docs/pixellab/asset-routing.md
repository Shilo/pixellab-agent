...
| Image edit, inpaint, remove background, resize, convert to pixel art | REST v2 edit/transform endpoints. | Supplied images are optional unless the selected route requires image fields. For fixed-size image-to-pixel-art output, use normal `image-to-pixelart` with `output_size`; `image-to-pixelart-pro` does not expose output dimensions. If Pro is needed, normalize afterward with `resize` when within its limit, or local nearest-neighbor/canvas postprocessing for larger backgrounds. |
