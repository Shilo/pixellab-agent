...
2. Classify the target asset or surface:
   ...
   Treat paperdoll items for an existing character, such as hair, eyes, hats, clothing, armor, accessories, held gear, or other body-part additions, as `existing_image` edit/paperdoll requests rather than standalone `object` requests unless the user explicitly asks for a separate unattached prop sprite.
...
4. Use MCP only if PixelLab MCP tools are available, either bare or prefixed. If MCP is unavailable, route to the matching REST v2 endpoint when one is documented. If MCP and REST v2 are both unavailable or fail, explain why before using any non-PixelLab fallback.
   ...
   PixelLab asset rule: every pixel of the requested art must originate from PixelLab or the user. Local tools may read, download, assemble, package, import/export, preview, verify, mask, pad, crop, resize, or format-convert those files. Do not draw, edit, or synthesize requested content locally unless the user explicitly requests or approves a labeled non-PixelLab fallback.
...
| Paperdoll item on an existing character, including hair, eyes, accessories, clothing, armor, hats, footwear, or held gear | Visible Aseprite `Edit image (Pro)` with output `New layer with changes` when the user wants an isolated layer in Aseprite; otherwise REST v2 `edit-images-v2` / Edit images (Pro) for a character-anchored edit. Prompt with character identity, direction, target body part, item placement, rotation/facing, occlusion, and preservation rules. | Do not use `create_map_object` or standalone object generation for fitted paperdoll layers unless the user explicitly asks for an unattached prop. Public REST image edits may return composites; only call a result a layer after verifying a changes-only layer or exported layer file. |
...
- "Paperdoll item": infer "item" broadly as any entity to add to a character body part, including hair, eyes, accessories, clothing, hats, armor, footwear, weapons, props, and VFX. For fitted/layered results, prefer image edit / Aseprite changes-only layer routes over object generation.
...
- Do not use local code or editor automation to create or alter requested visual content for PixelLab asset requests. Examples: PIL/Pillow, canvas/SVG, ImageMagick draw commands, Aseprite Lua drawing, ASCII-to-image, and procedural pixel placement.
